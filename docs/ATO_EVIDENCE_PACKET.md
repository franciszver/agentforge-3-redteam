...
`pytest tests/ -q` re-run for this packet: **327 passed** with the sibling
Phase 2 checkout (`../agentforge-2-evidence-agent`, pinned `v2.0.0`)
present locally (confirmed at PR time); **227 passed, 100 skipped** in CI
...
- **327 passing tests (227 passed, 100 skipped in CI), no live/network/GPU
  ...
  changes plus its expanded citation-verification test set move it to 327
  with the sibling checkout present, or 227 passed / 100 skipped without
  it, §5.1).
...
### 5.1 The 327-test suite (227 in CI)

`pytest tests/ -q` → **327 passed** with the sibling Phase 2 checkout
present, re-confirmed for this packet (§4.1); **227 passed, 100 skipped**
...
- **8 deterministic campaign tests** (fake clients, no live calls): full
  ...
  or 227 passed / 100 skipped without it, §5.1).
...
- **All six required sections present:** §1 Architecture and data-flow
  ...
  evidence), §5 Eval-result evidence (327 tests with the sibling checkout
  present / 227 passed, 100 skipped in CI, 3 criticals, live-campaign
