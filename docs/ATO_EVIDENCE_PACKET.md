...
`pytest tests/ -q` re-run for this packet: **249 passed** with the sibling
Phase 2 checkout (`../agentforge-2-evidence-agent`, pinned `v2.0.0`)
present locally (confirmed at PR time); **209 passed, 40 skipped** in CI
...
- **249 passing tests (209 passed, 40 skipped in CI), no live/network/GPU
  ...
  changes plus its expanded citation-verification test set move it to 249
  with the sibling checkout present, or 209 passed / 40 skipped without
  it, §5.1).
...
### 5.1 The 249-test suite (209 in CI)

`pytest tests/ -q` → **249 passed** with the sibling Phase 2 checkout
present, re-confirmed for this packet (§4.1); **209 passed, 40 skipped**
...
- **8 deterministic campaign tests** (fake clients, no live calls): full
  ...
  the repo has since grown to 249 total with the sibling checkout present,
  or 209 passed / 40 skipped without it, §5.1).
...
- **All six required sections present:** §1 Architecture and data-flow
  ...
  evidence), §5 Eval-result evidence (249 tests with the sibling checkout
  present / 209 passed, 40 skipped in CI, 3 criticals, live-campaign
