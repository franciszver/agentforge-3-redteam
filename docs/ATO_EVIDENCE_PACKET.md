...
`pytest tests/ -q` re-run for this packet: **343 passed** with the sibling
Phase 2 checkout (`../agentforge-2-evidence-agent`, pinned `v2.0.0`)
present locally (confirmed at PR time); **237 passed, 106 skipped** in CI
...
- **343 passing tests (237 passed, 106 skipped in CI), no live/network/GPU
  ...
  changes plus its expanded citation-verification test set move it to 343
  with the sibling checkout present, or 237 passed / 106 skipped without
  it, §5.1).
...
### 5.1 The 343-test suite (237 in CI)

`pytest tests/ -q` → **343 passed** with the sibling Phase 2 checkout
present, re-confirmed for this packet (§4.1); **237 passed, 106 skipped**
...
- **All six required sections present:** §1 Architecture and data-flow
  ...
  evidence), §5 Eval-result evidence (343 tests with the sibling checkout
  present / 237 passed, 106 skipped in CI, 3 criticals, live-campaign
