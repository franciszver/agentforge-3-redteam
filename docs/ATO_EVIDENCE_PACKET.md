...
`pytest tests/ -q` re-run for this packet: **405 passed** with the sibling
Phase 2 checkout (`../agentforge-2-evidence-agent`, pinned `v2.0.0`)
present locally (confirmed at PR time); **299 passed, 106 skipped** in CI
...
- **405 passing tests (299 passed, 106 skipped in CI), no live/network/GPU
  ...
  changes plus its expanded citation-verification test set move it to 405
  with the sibling checkout present, or 299 passed / 106 skipped without
  it, §5.1).
...
### 5.1 The 405-test suite (299 in CI)

`pytest tests/ -q` → **405 passed** with the sibling Phase 2 checkout
present, re-confirmed for this packet (§4.1); **299 passed, 106 skipped**
...
- **8 deterministic campaign tests** (fake clients, no live calls): full
  ...
  the repo has since grown to 405 total with the sibling checkout present,
  or 299 passed / 106 skipped without it, §5.1).
...
- **All six required sections present:** §1 Architecture and data-flow
  ...
  evidence), §5 Eval-result evidence (405 tests with the sibling checkout
  present / 299 passed, 106 skipped in CI, 3 criticals, live-campaign
