...
- `pytest tests/ -q` green (deterministic — no live/network/GPU call in the
  ...
  environment-dependent: **250 passed** when the sibling Phase 2 checkout
  (`../agentforge-2-evidence-agent`, pinned `v2.0.0`) is present locally;
  **210 passed, 40 skipped** in CI and for anyone cloning this repo without
...
```
$ pytest tests/ -q
250 passed in 0.63s          # with the sibling Phase 2 checkout present
```
...
count is **210 passed, 40 skipped** (the 40 skipped are
...
checkout present, giving **250 passed**. `pytest tests/ -q` is still the
reproducibility bar — re-run it after pulling this branch to confirm
nothing here has drifted: expect **250 passed** if you have the sibling
Phase 2 checkout at `v2.0.0`, or **210 passed, 40 skipped** if you don't.
