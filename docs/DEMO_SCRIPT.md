...
- `pytest tests/ -q` green (deterministic — no live/network/GPU call in the
  ...
  environment-dependent: **352 passed** when the sibling Phase 2 checkout
  (`../agentforge-2-evidence-agent`, pinned `v2.0.0`) is present locally;
  **246 passed, 106 skipped** in CI and for anyone cloning this repo without
  ...
  `tests/test_v210_upstream_status.py`, issue #58) plus
  `TestStandingUpTargetPathsExistInPinnedTarget` (6 cases,
  `tests/test_claude_md_accuracy.py`, issue #61) — all three class-skip
  cleanly when the sibling is absent).

```
$ pytest tests/ -q
352 passed in 2.38s          # with the sibling Phase 2 checkout present
```
...
count is **246 passed, 106 skipped** (the 106 skipped are
...
checkout present, giving **352 passed**. `pytest tests/ -q` is still the
reproducibility bar — re-run it after pulling this branch to confirm
nothing here has drifted: expect **352 passed** if you have the sibling
Phase 2 checkout at `v2.0.0`, or **246 passed, 106 skipped** if you don't.
