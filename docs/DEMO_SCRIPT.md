...
- `pytest tests/ -q` green (deterministic — no live/network/GPU call in the
  ...
  environment-dependent: **249 passed** when the sibling Phase 2 checkout
  (`../agentforge-2-evidence-agent`, pinned `v2.0.0`) is present locally;
  **209 passed, 40 skipped** in CI and for anyone cloning this repo without
...
```
$ pytest tests/ -q
249 passed in 0.63s          # with the sibling Phase 2 checkout present
```
...
count is **209 passed, 40 skipped** (the 40 skipped are
...
checkout present, giving **249 passed**. `pytest tests/ -q` is still the
reproducibility bar — re-run it after pulling this branch to confirm
nothing here has drifted: expect **249 passed** if you have the sibling
Phase 2 checkout at `v2.0.0`, or **209 passed, 40 skipped** if you don't.
