...
observed-vs-expected, remediation, fix-validation status, and (issue #77/
P3.36) `recording_ref`: `vuln_report.schema.json` is `additionalProperties:
false` and deliberately has no `minimal_repro` field (see
`redteam/agents/documentation.py`'s own module docstring), but its optional
`recording_ref` property IS a report field — a directory under
`evals/recordings/` naming the committed, replayable draw(s) that back the
finding, derived by `build_vuln_report()` from the source exploit record's
own `recording_ref`, never hand-typed and never narrator-overridable — so a
filed report resolves to its own evidence directly, reproducible by an
engineer with zero platform context from the report alone plus the
committed recording under `evals/recordings/`.
...
4. **Documentation Agent.** Converts Judge-confirmed exploits into structured
   ...
   observed-vs-expected, remediation, fix-validation status, `recording_ref`;
   the schema has no `minimal_repro` field, see
   `redteam/agents/documentation.py`'s module docstring) that a fresh
   engineer can act on, without platform context, via the report's own
   `recording_ref` (issue #77/P3.36) plus the committed recording under
   `evals/recordings/` it names. A human-approval gate sits
